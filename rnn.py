...
class HBVMulTDET_DA6(torch.nn.Module):
    ...
    def __init__(self):
        ...
        super(HBVMulTDET_DA6, self).__init__()

    def forward(self, x, q_warm, parameters, initial_S,staind, tdlst, mu, muwts, rtwts, bufftime=0, outstate=False, routOpt=False,
        ...
        SNOWPACK,MELTWATER,SM,SUZ,SLZ = initial_S
        ...
        SNOWMPACK_simmu = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).cuda()
        SM_simmu = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).cuda()
        SLZ_simmu = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).cuda()
        SUZ_simmu = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).cuda()
        MELTWATER_simmu = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).cuda()
        ...
        for t in range(Nstep):
            ...
            Qsimmu2[t, :, :] = Q2
            ETmu[t, :, :] = ETact
            SNOWMPACK_simmu[t, :, :] = SNOWPACK
            SM_simmu[t, :, :] = SM
            SLZ_simmu[t, :, :] = SLZ
            SUZ_simmu[t, :, :] = SUZ
            MELTWATER_simmu[t, :, :]=MELTWATER
        ...
        Snow_DA = SNOWMPACK_simmu.mean(-1, keepdim=True)
        SM_DA = SM_simmu.mean(-1,keepdim=True)
        SUZ_DA = SUZ_simmu.mean(-1,keepdim=True)
        SLZ_DA = SLZ_simmu.mean(-1,keepdim=True)
        MELTWATER_DA = MELTWATER_simmu.mean(-1,keepdim=True)
        ...
        if routOpt is True: # routing
            ...
            routa = tempa.repeat(30, 1).unsqueeze(-1)
            routb = tempb.repeat(30, 1).unsqueeze(-1)
            UH = UH_gamma(routa, routb, lenF=15)  # lenF: folter
            Qsim2 = torch.cat((q_warm, Qsim), dim=0)
            rf = torch.unsqueeze(Qsim2, -1).permute([1, 2, 0])   # dim:gage*var*time
        ...
        if outstate is True: # output states
            ...
            # return Qs
            Qall = torch.cat((Qs, SM_DA, SUZ_DA, SLZ_DA, Snow_DA), dim=-1)
            return Qall
